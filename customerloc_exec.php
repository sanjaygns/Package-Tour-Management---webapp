...
$q = "select p_no from location where l_name = '$location' union select p_no from subplaces where place_name = '$location'";
$pnores = mysql_query($q);
if (mysql_num_rows($pnores) < 1)
{
	?>
	<script>
		if(!alert("NO package found")) document.location = 'customerloc.php'
	</script>
	<?php	
}	
...
<div align="center"><span class="style10">List of Available Package Names</span>
...
<?php 
$sa = 0;
while ($r = mysql_fetch_array($pnores))
{	
	$pno = $r['p_no'];
	$q= "select * from package where p_no = '$pno' and DATE(dep_date) >= CURDATE()";
	$pack = mysql_query($q);
	$name = mysql_fetch_array($pack);
	if(($r['total_seats'] - $r['seats_booked']) > 0)
	{
		$sa++;
?> <p align="center" class="style11"><?php echo $sa.'. '.$name['p_name']; ?> </p>
